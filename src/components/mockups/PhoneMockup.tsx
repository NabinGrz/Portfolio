...
export const PhoneMockup: React.FC<PhoneMockupProps> = ({
  ...
  const [viewMode, setViewMode] = useState<'screenshot' | 'interactive'>(image ? 'screenshot' : 'interactive');
  const [checkedTask, setCheckedTask] = useState<boolean>(true);

  const currentScreen = interactive ? activeTab : screenType;

  // Screenshot image map for interactive switcher if no explicit image prop passed
  const defaultImages: Record<ScreenType, string> = {
    citypay: '/images/portfolio_01.jpg',
    namaste: '/images/portfolio_02.jpg',
    agri: '/images/portfolio_03.jpg',
    cronlink: '/images/portfolio_04.jpg',
    kharcha: '/images/portfolio_05.jpg',
  };

  const activeImage = image || defaultImages[currentScreen];
    ...
    <div className={`relative mx-auto select-none ${className}`}>
      ...
      <div className="relative w-[300px] sm:w-[325px] md:w-[340px] h-[640px] sm:h-[675px] bg-[#121620] rounded-[52px] p-[10px] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)] ring-1 ring-white/20 border border-slate-700/80">
        ...
        <div className="relative w-full h-full bg-[#080c14] rounded-[42px] overflow-hidden flex flex-col border border-slate-800 shadow-inner">
          ...
          {/* Status Bar */}
          <div className="pt-2.5 px-6 pb-2 flex items-center justify-between text-[11px] font-semibold text-slate-300 z-30 bg-[#080c14]/90 backdrop-blur-md">
          ...
          {/* Screen Content: Screenshot Image OR Interactive Simulator */}
          <div className="flex-1 overflow-hidden relative text-left">
            {viewMode === 'screenshot' && activeImage ? (
              <div className="relative w-full h-full bg-slate-950 flex items-center justify-center overflow-hidden">
                ...
                />
                {/* Subtle bottom gradient to blend with home indicator */}
                <div className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-t from-slate-950 to-transparent pointer-events-none" />
              </div>
            ) : (
              <div className="flex-1 h-full overflow-y-auto px-4 py-2 relative scrollbar-none">
          ...
          {/* Interactive Screen Selector Pills & View Toggle */}
          {interactive && (
            <div className="px-2.5 py-2 bg-slate-950 border-t border-white/10 flex items-center justify-between text-[9px] z-20">
              ...
              {/* Toggle between Original Screenshot & Interactive Mockup */}
              <button
                type="button"
                onClick={() => setViewMode(viewMode === 'screenshot' ? 'interactive' : 'screenshot')}
                className="ml-1 px-2 py-1 rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700 font-mono font-bold text-[8px] shrink-0 flex items-center gap-1 border border-white/10"
                title="Toggle Screenshot / Interactive"
              >
                <ImageIcon className="w-2.5 h-2.5 text-orange-400" />
                <span>{viewMode === 'screenshot' ? 'Sim' : 'Shot'}</span>
              </button>
            </div>
          )}
